...
func (p *RPCClientV6) ListImagesWithOptions(ctx context.Context, opts v10.ListImagesOptions) ([]v6.ImageStatus, error) {
	...
	for i, image := range images {
		for j, container := range image.Containers {
			tagPattern := daemon.GetTagPattern(policyMap, image.ID, container.Name)
			// Build a new container using the same function used in v10
			newContainer, err := daemon.BuildContainer(container.Name, container.Available, container.Current, tagPattern, opts.OverrideContainerFields)
			if err != nil {
				return images, err
			}
